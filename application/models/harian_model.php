...
class Harian_model extends CI_Model {
    ...
    function main_query($tanggal = 0) {
        ...
        if ($tanggal == 0)
            $query = $this->db->query('SELECT * FROM harian WHERE tanggal = "' . $yesterday . '" ORDER BY kebun DESC');
        else
            $query = $this->db->query('SELECT * FROM harian WHERE tanggal = "' . $tanggal . '" ORDER BY kebun DESC');
        return $query;
    }
	
	function get_daily($kebun = 0){
		$yesterday = date("Y-m-d", time() - 86400);
		$query = $this->db->query('SELECT * FROM harian WHERE tanggal = "' . $yesterday . '" AND kebun = "' . $kebun . '" ORDER BY kebun DESC');
        return $query->row();
	}
	
	function get_monthly($kebun = 0){
		$yesterday = date("Y-m-d", time() - 86400);
		$str = substr($yesterday, 0, 7);
		$query = $this->db->query('SELECT SUM(rkap) AS rkap, SUM(rko) AS rko, SUM(realisasi) AS realisasi FROM harian WHERE SUBSTRING(tanggal,1,7) = "'.$str.'" AND kebun = "' . $kebun . '" ORDER BY kebun DESC');
        return $query->row();
	}
	
	function get_yearly($kebun = 0){
		$yesterday = date("Y-m-d", time() - 86400);
		$str = substr($yesterday, 0, 4);
		$query = $this->db->query('SELECT SUM(realisasi) AS realisasi, SUM(rkap) AS rkap, SUM(rko) AS rko FROM harian WHERE SUBSTRING(tanggal,1,4) = "'.$str.'" AND kebun = "' . $kebun . '" ORDER BY kebun DESC');
        return $query->row();
	}
