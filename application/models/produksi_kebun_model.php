 ...
 */
class Produksi_kebun_model extends CI_Model {
    ...
    function get_rows_produksi($kebun, $tgl){
        $query = $this->db->query('SELECT * FROM produksi_kebun WHERE kebun = "'. $kebun .'" AND tanggal = "'. $tgl .'"');
        return $query->num_rows();
    }

    function get_rows_realisasi($kebun, $tgl){
        $query = $this->db->query('SELECT * FROM produksi_kebun WHERE kebun = "'. $kebun .'" AND tanggal = "'. $tgl .'" AND realisasi IS NOT NULL');
        return $query->num_rows();
    }

    function get_rows_estimasi($kebun, $tgl){
        $query = $this->db->query('SELECT * FROM produksi_kebun WHERE kebun = "'. $kebun .'" AND tanggal = "'. $tgl .'" AND estimasi IS NOT NULL');
        return $query->num_rows();
    }

    function get_rows_brd($kebun, $tgl){
        $query = $this->db->query('SELECT * FROM produksi_kebun WHERE kebun = "'. $kebun .'" AND tanggal = "'. $tgl .'" AND brondolan IS NOT NULL');
        return $query->num_rows();
    }

    function get_rows_hk($kebun, $tgl){
        $query = $this->db->query('SELECT * FROM produksi_kebun WHERE kebun = "'. $kebun .'" AND tanggal = "'. $tgl .'" AND hk_dinas IS NOT NULL');
        return $query->num_rows();
    }
    
    function get_rows_sisa($kebun, $tgl){
        $query = $this->db->query('SELECT * FROM produksi_kebun WHERE kebun = "'. $kebun .'" AND tanggal = "'. $tgl .'" AND sisa IS NOT NULL');
        return $query->num_rows();
    }
    
    function get_rows_ch($kebun, $tgl){
        $query = $this->db->query('SELECT * FROM produksi_kebun WHERE kebun = "'. $kebun .'" AND tanggal = "'. $tgl .'" AND curah_hujan IS NOT NULL');
        return $query->num_rows();
    }
    
    function get_rows_tel($kebun, $tgl){
        $query = $this->db->query('SELECT * FROM produksi_kebun WHERE kebun = "'. $kebun .'" AND tanggal = "'. $tgl .'" AND telling IS NOT NULL');
        return $query->num_rows();
    }

    function insert_realisasi($tgl, $kebun, $realisasi){
        $this->tanggal = $tgl;
        $this->kebun = $kebun;
        $this->realisasi = $realisasi;

        $this->db->insert('produksi_kebun', $this);
        return $this->get_rows_realisasi($kebun, $tgl);
    }

    function update_realisasi($tgl, $kebun, $realisasi){
        $this->realisasi = $realisasi;

        $this->db->update('produksi_kebun', $this, array('tanggal' => $tgl, 'kebun' => $kebun));
        return $this->get_rows_realisasi($kebun, $tgl);
    }

    function insert_estimasi($tgl, $kebun, $estimasi){
        $this->tanggal = $tgl;
        $this->kebun = $kebun;
        $this->estimasi = $estimasi;

        $this->db->insert('produksi_kebun', $this);
        return $this->get_rows_estimasi($kebun, $tgl);
    }

    function update_estimasi($tgl, $kebun, $estimasi){
        $this->estimasi = $estimasi;
    ...
    function update_brd($tgl, $kebun, $afdeling, $brd){
        ...
        $this->db->update('produksi_kebun', $this, array('tanggal' => $tgl, 'kebun' => $kebun, 'afdeling' => $afdeling));
        return $this->get_rows_brd($kebun, $afdeling, $tgl);
    }
    ...
    function insert_sisa($tgl, $kebun, $sisa){
        $this->tanggal = $tgl;
        $this->kebun = $kebun;
        $this->sisa = $sisa;

        $this->db->insert('produksi_kebun', $this);
        return $this->get_rows_sisa($kebun, $tgl);
    }

    function update_sisa($tgl, $kebun, $sisa){
        $this->sisa = $sisa;
    ...
    function insert_all($tgl, $kebun, $arr){
        $this->tanggal = $tgl;
        $this->kebun = $kebun;
        $tel = substr($arr[0], 8);
        $this->estimasi = trim($tel);
        $this->realisasi = trim($arr[1]);
        $this->sisa = trim($arr[2]);

        $this->db->insert('produksi_kebun', $this);
        return $this->get_rows_realisasi($kebun, $tgl);
    }

    function update_all($tgl, $kebun, $arr){
        $tel = substr($arr[0], 8);
        $this->estimasi = trim($tel);
        $this->realisasi = trim($arr[1]);
        $this->sisa = trim($arr[2]);
